...
#define GD25_COMMAND_READ_SR3       0x15

#define GD25_P_FAIL                 (0x1 << 2)
#define GD25_IS_PROGRAM_FAIL(sec)   (((sec)&GD25_P_FAIL) != 0)

#define GD25_E_FAIL             (0x1 << 3)
#define GD25_IS_ERASE_FAIL(sec) (((sec)&GD25_E_FAIL) != 0)

/*
 * gd25_nor_get_program_result() - Check the programming result of
 * the SPI NOR flash.
...
int gd25_nor_get_program_result(struct xh2a_qspi_dev *qspi_dev)
{
	...
	xh2a_qspi_writel(qspi_dev, DW_SPI_DR, GD25_COMMAND_READ_SR3);

	usleep_range(POLL_DELAY_MIN, POLL_DELAY_MAX);

	xh2a_qspi_readl(qspi_dev, DW_SPI_DR, &status);

	if (GD25_IS_PROGRAM_FAIL(status)) {
...
/*
 * gd25_nor_get_erase_result() - Check the result of the SPI NOR flash
 * erase operation.
 * @qspi_dev: QSPI device handle
